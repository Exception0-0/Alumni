...
class EmailSignUpUseCase @Inject constructor(private val repository: EmailAuthRepository) :
    UseCase<EmailAuthParam, Unit> {
    override suspend fun invoke(param: EmailAuthParam): Either<Failure, Unit> {
        // TODO: add validation
        return when (val result = repository.signUp(param.email, param.password)) {
