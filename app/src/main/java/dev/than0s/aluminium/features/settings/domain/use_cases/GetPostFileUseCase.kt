...
class GetPostFileUseCase @Inject constructor(private val repository: StorageRepository) :
    UseCase<String, Uri> {
    override suspend fun invoke(fileId: String): Either<Failure, Uri> {
        return when (val result = repository.getFile(getPostsFilePath(fileId))) {
